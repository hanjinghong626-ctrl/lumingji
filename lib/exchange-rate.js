...
export async function getExchangeRates() {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      // 主API失败，尝试 frankfurter.app
      return await getAlternativeRates();
    }
...
/**
 * 备用API：frankfurter.app（欧洲央行数据，免费无key，支持CORS）
 */
async function getAlternativeRates() {
  try {
    const response = await fetch('https://api.frankfurter.app/latest?from=CNY', {
      signal: AbortSignal.timeout(6000),
    });
    if (!response.ok) return await getFallbackRates();
    const data = await response.json();
    if (!data.rates) return await getFallbackRates();
    const rates = {
      base: 'CNY',
      date: data.date,
      rates: data.rates,
